$.getJSON("http://cpv2api.com/posts/published/j-w-v", function(resp) {
  if (resp.success) {
    for (var i = 0; i < resp.data.length; i++) {
      $('.posts ul').append('<li class="poop"><a target="_blank" href="' + resp.data[i].link + '">' + resp.data[i].title + ' <span> ' + resp.data[i].views + ' views</span> </a></li>');
    }
  }
});
$.getJSON("http://cpv2api.com/pens/showcase/j-w-v", function(resp) {
  if (resp.success) {
    for (var i = 0; i < 5; i++) {
      $('.pens ul').append('<li class="poop"><a target="_blank" href="' + resp.data[i].link + '">' + resp.data[i].title + ' <span> ' + resp.data[i].views + ' views</span> </a></li>');
    }
  }
});
...
$("#set div").draggable({
  ...
  stop: function(event, ui) {
    ...
    if (nodeID === 'draggableTEXT') {
      ...
      newNode = '<img src=\'http://wristgeek.com/wp-content/uploads/2014/09/hello_world.png\'/>';
      $result.append(newNode);
    } else if (nodeID === 'draggableCOL') {
      newNode = '<div class=\'sortable row\' id=\'newHalf\'></div>';
      $result.append(newNode);
